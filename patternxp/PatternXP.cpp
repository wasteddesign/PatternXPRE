...
void mi::Update(CSubTickInfo const *psti)
{
	...
	// Follow pattern
	/*
	if (patEd->pPattern != NULL &&
		pCB->GetProfileIntA("FollowPattern", 0) == 1 &&
		patEd->dialogOpen == false)
	{
		CMachine* mac = pCB->GetPatternOwner(patEd->pPattern->pPattern);
		CSequence* seq = pCB->GetPlayingSequence(mac);
		CPattern* playingPattern = NULL;
		if (seq != NULL)
			playingPattern = pCB->GetPlayingPattern(seq);

		if (playingPattern != NULL && patEd->pPattern->pPattern != playingPattern)
		{
			ex.SetEditorPattern(playingPattern);
		}
	}
	*/
