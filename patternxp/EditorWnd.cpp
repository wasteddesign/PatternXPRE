...
BEGIN_MESSAGE_MAP(CEditorWnd, CWnd)
	...
	ON_BN_CLICKED(IDC_FOLLOW_PLAY_POS, &CEditorWnd::OnBnClickedFollowPlayPos)
	ON_BN_CLICKED(IDC_FOLLOW_PLAYING_PATTERN, &CEditorWnd::OnBnClickedFollowPlayingPattern)
END_MESSAGE_MAP()
...
*/
	...
	//pe.ModifyStyleEx(WS_EX_CLIENTEDGE, WS_EX_STATICEDGE, SWP_DRAWFRAME);
	...
	//topwnd.ModifyStyleEx(WS_EX_CLIENTEDGE, WS_EX_STATICEDGE, SWP_DRAWFRAME);
	...
	//leftwnd.ModifyStyleEx(WS_EX_CLIENTEDGE, WS_EX_STATICEDGE, SWP_DRAWFRAME);
	...
	//topleftwnd.ModifyStyleEx(WS_EX_CLIENTEDGE, WS_EX_STATICEDGE, SWP_DRAWFRAME);
	topleftwnd.pCB = pCB;
	...
	followPlayPos = this->pCB->GetProfileIntA("FollowPlayPosition", 0) == 1;
	followPattern = this->pCB->GetProfileIntA("FollowPattern", 0) == 1;

	SetFollowPlayPos(followPlayPos);
	SetFollowPlayingPattern(followPattern);
...
{
	bool toolbarsEnabled = this->pCB->GetProfileIntA("ToolBars", 1) == 1;
	int state = SW_SHOW;
	if (!toolbarsEnabled)
	{
		state = SW_HIDE;
	...
	CWnd* wnd = GetParent()->GetParent()->GetDlgItem(EDITOR_MAIN_TOOLBAR);
	if (wnd != NULL)
	{
		wnd->ShowWindow(state);
	}

	dlgBar.ShowWindow(state); // Works!
...
void CEditorWnd::SetFollowPlayPos(bool follow, bool save)
{	
	if (save)
		this->pCB->WriteProfileInt("FollowPlayPosition", (int)follow);

	CButton* pc = (CButton*)dlgBar.GetDlgItem(IDC_FOLLOW_PLAY_POS);
	if (pc != NULL)
	{
		if (follow)
...
void CEditorWnd::SetFollowPlayingPattern(bool follow, bool save)
{
	if (save)
		this->pCB->WriteProfileInt("FollowPattern", (int)follow);
...
void CEditorWnd::FontChanged()
{
	...
	topWndHeight = 144 * DPIScale(GetDC());// 3 * fontSize.cy + 4;
...
void CEditorWnd::ToggleMidiEdit()
{
	CButton* pc = (CButton*)dlgBar.GetDlgItem(IDC_MIDI_EDIT);
...
void CEditorWnd::OnBnClickedFollowPlayPos()
{
	CButton* pc = (CButton*)dlgBar.GetDlgItem(IDC_FOLLOW_PLAY_POS);
	followPlayPos = pc->GetCheck() == BST_CHECKED;
	this->pCB->WriteProfileInt("FollowPlayPosition", (int)followPlayPos);
}

void CEditorWnd::OnBnClickedFollowPlayingPattern()
{
	CButton* pc = (CButton*)dlgBar.GetDlgItem(IDC_FOLLOW_PLAYING_PATTERN);
	followPattern = pc->GetCheck() == BST_CHECKED;
	this->pCB->WriteProfileInt("FollowPattern", (int)followPattern);
}
